...
export const isValidJSON = (data) => {
  ...
  try {
    ...
    if (
      !('achievements' in keys) && !Array.isArray(newResume.achievements)
    ) {
      throw new Error('');
    }
    cleanedResume = {
...
export const ACHIEVEMENTS = 4;

export const defaultResumeOrder = [EDUCATION, TECH_SKILLS, PROJECTS, EXPERIENCE, ACHIEVEMENTS];
